...
void covarLoopaCpu(DATA_TYPE pmean[M+1], DATA_TYPE pdata[M+1][N+1], DATA_TYPE pfloat_n)
{
	int i, j, j1, j2;
...
void covarLoopbCpu(DATA_TYPE pdata[M+1][N+1], DATA_TYPE pmean[M+1])
{
	int i, j;
...
void covarLoopcCpu(DATA_TYPE psymmat[M+1][N+1], DATA_TYPE pdata[M+1][N+1])
{
	int i, j1, j2;
...
int main(int argc, char** argv)
{
	...
	covarLoopaCpu(mean, data, float_n);
	covarLoopbCpu(data, mean);
	covarLoopcCpu(symmat, data);
